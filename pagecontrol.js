	...
	if (i < 8) {
		...
		str += '<li><a href="javascript:void(0)" id="pagedown" onclick="pagedown(' + count
			+ ')">&raquo;</a></li><span id="count_content">(共<span id="count">'
				+ count + '</span>条记录)<input style="opacity: 0; width: 4.5%;"/>跳转到:<input id="gotiao" type="text" style="width:3.5%;color:#000000;"/> 页<button type="button" style="width:3%;margin-left:1%" onclick="tiaozhuan();">GO</button></span>';
...
function huanye(num, count) {
	...
	// 当画面最大页码数打于8页的情况下
		...
		// 当用户点击“...”省略页码，分页控件需要更改式样
			...
			str += '<li><a href="javascript:void(0)" id="pagedown" onclick="pagedown(' + count
					+ ')">&raquo;</a></li><span id="count_content">(共<span id="count">'
					+ count + '</span>条记录)<input style="opacity: 0; width: 5.5%;"/>跳转到:<input id="gotiao" type="text" style="width:5.5%;color:#000000;"/> 页<button type="button" style="width:5%;margin-left:1%;" onclick="tiaozhuan();">GO</button></span>';
			$(".pagination").html(str);
			// 用户所点击的页码，样式变更
			$("#page_" + num).addClass("active");
		}
